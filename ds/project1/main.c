...
void main()
{
	...
	printf("\033[33;1m            a/A)add new record\n            d/D)delete a record\n            s/S)show the list\n            m/M)modify a record\n            v/V)save\n            e/E)exit\n            t/T)sort the list\n            l/L)delete all records\n            r/R)reverse the list\n            b/B)show list batch-wise from file\n            f/F)search record\n            n/N)show list batch-wise\n            c/C)count the records\n\033[0m");
	printf("         \033[36;1m **********************************\n\033[0m");
	while(1)
	{
		...
		switch(ch)
		{
			...
			case 'F': stud_search(head);	break;
		//	case 'B': stud_batchwise_file(head); break;
		//	case 'N': stud_batchwise(head);      break;
			case 'C': stud_count(head);          break;
